...
export function LessonCanvas() {
  ...
  if (!activeLesson) {
    return (
      <div className="flex-1 bg-night h-full flex flex-col items-center justify-center">
        <div className="text-center space-y-3 max-w-[320px]">
          <div className="w-16 h-16 mx-auto rounded-2xl bg-white/[0.035] border border-white/[0.08] flex items-center justify-center">
            <FileText className="h-7 w-7 text-chalk-dim" />
          </div>
          <h3 className="text-[15px] font-semibold text-chalk">No lesson selected</h3>
  ...
  return (
    <div className="flex-1 bg-night h-full overflow-y-auto relative">
      <div className="max-w-[780px] mx-auto py-8 px-8 space-y-6 pb-40">
        {/* Lesson Header */}
        <div className="flex items-center justify-between bg-night p-5 rounded-2xl border border-white/[0.08]">
          <div className="flex items-center gap-3 flex-1">
            <span className="text-[14px] font-semibold text-chalk-muted shrink-0">
              {chIdx}.{lIdx}
            </span>
            {isEditingTitle ? (
              <input
                ...
                className="text-[26px] font-bold text-chalk bg-transparent border-b-2 border-lime outline-none w-full"
              />
            ) : (
              <h1
                className="text-[26px] font-bold text-chalk cursor-text hover:text-lime transition-colors"
        ...
        {/* Lesson Description */}
        <div className="bg-night border border-white/[0.08] rounded-2xl p-5 space-y-2">
          <label className="text-[11px] font-semibold text-chalk-muted uppercase tracking-wider">
            Lesson Description
          </label>
          <textarea
            className="w-full bg-transparent text-[14px] text-chalk placeholder:text-chalk-muted resize-none focus:outline-none min-h-[60px]"
        ...
        <div className="relative" ref={addMenuRef}>
          <button
            ...
            className="w-full border-2 border-dashed border-white/[0.08] hover:border-lime/50 rounded-2xl py-4 flex items-center justify-center gap-2 text-[13px] font-medium text-chalk-muted hover:text-lime hover:bg-lime/10/30 transition-all"
          >
          ...
          {showAddMenu && (
            <div className="absolute left-0 right-0 top-full mt-2 bg-night border border-white/[0.08] rounded-2xl p-3 shadow-float z-50 grid grid-cols-4 gap-1.5">
              {BLOCK_TYPES.map((bt) => (
                <button
                  key={bt.type}
                  onClick={() => addBlock(bt.type)}
                  className="flex flex-col items-center justify-center p-3 rounded-xl gap-1.5 hover:bg-white/[0.035] transition-colors group"
                >
                  <div className="w-9 h-9 rounded-lg bg-white/[0.035] border border-white/[0.08] flex items-center justify-center group-hover:border-lime/30 group-hover:bg-lime/10 transition-all">
                    <bt.icon className="h-4 w-4 text-chalk-muted group-hover:text-lime" />
                  </div>
                  <span className="text-[9px] font-medium text-chalk-muted group-hover:text-chalk">
                    {bt.label}
                  </span>
                </button>
              ))}
        ...
        {/* AI Prompt */}
        <div className="bg-gradient-to-r from-lime/10 to-emerald-500/10 border border-lime/20 rounded-2xl p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-lime flex items-center justify-center">
              <Sparkles className="h-4 w-4 text-white" />
            </div>
            <div>
              <p className="text-[12px] font-semibold text-chalk">Generate content with AI</p>
              <p className="text-[10px] text-chalk-muted">
                Describe what you want and AI will write it for you
              </p>
            </div>
          </div>
          <Button size="sm" className="h-7 rounded-lg text-[11px] bg-lime hover:bg-lime/90 text-white">
            Write with AI
          </Button>
        </div>
      </div>
...
function BlockWrapper({
  ...
  return (
    <div
      className={`group/block relative bg-night rounded-2xl border transition-all ${
        isActive ? "border-lime/50 shadow-sm ring-1 ring-lime/20" : "border-white/[0.08] hover:border-white/[0.08]"
      }`}
      onClick={onActivate}
    >
      ...
      <div className="absolute -left-10 top-2 flex flex-col gap-0.5 opacity-0 group-hover/block:opacity-100 transition-opacity">
        <button
          className="p-1 rounded-md bg-night border border-white/[0.08] hover:bg-white/[0.035] text-chalk-muted"
      ...
      <div className="absolute top-2 right-2 flex items-center gap-0.5 opacity-0 group-hover/block:opacity-100 transition-opacity">
        <button
          className="p-1 rounded-md hover:bg-white/[0.035] text-chalk-muted"
          onClick={(e) => { e.stopPropagation(); onDuplicate(); }}
        >
          <Copy className="h-3 w-3" />
        </button>
        <button
          className="p-1 rounded-md hover:bg-rose-500/10 text-chalk-muted hover:text-rose-500"
      ...
      {/* Block Type Label */}
      <div className="absolute -top-2.5 left-4 px-2 py-0.5 bg-night border border-white/[0.08] rounded-full text-[9px] font-semibold text-chalk-muted uppercase tracking-wider">
        {block.type}
      </div>
    </div>
  );
