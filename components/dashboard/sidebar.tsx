...
export function Sidebar({
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 flex w-64 flex-col border-r border-line bg-paper/85 backdrop-blur-xl">
      {/* Brand Header */}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 space-y-8">
        {/* User Profile Card */}
        <div className="px-3">
          <div className="flex items-center gap-3 p-2 rounded-2xl border border-line bg-paper-soft shadow-2xl shadow-black/20">
                ...
                <div className="flex items-center gap-3">
                  <item.icon
                    className={cn(
                      ...
                      isActive ? "text-night" : "text-ink-subtle group-hover:text-ink"
                    )}
                  />
                  {item.label}
                </div>
          ...
          <div className="space-y-0.5">
            {SHORTCUTS.map((s) => (
              <Link
                key={s.label}
                href={s.href}
                className="flex items-center justify-between p-2 rounded-xl text-[12.5px] font-medium text-ink-muted hover:bg-paper-muted hover:text-ink transition-all group"
              >
                <div className="flex items-center gap-3">
        ...
        <div className="px-3 space-y-3 pt-2">
          ...
          <div className="p-3 rounded-2xl border border-line bg-paper-soft space-y-4 relative overflow-hidden">
            ...
            <Link href="/dashboard/settings" className="flex items-start gap-3 w-full group relative z-10">
               <div className="h-4 w-4 rounded-full border border-line-strong bg-paper flex items-center justify-center shrink-0 mt-0.5 group-hover:border-lime/50 group-hover:bg-lime/10 transition-colors">
                  <div className="h-1.5 w-1.5 rounded-full bg-line-strong group-hover:bg-lime transition-colors" />
               </div>
               <div className="flex-1">
                  <p className="text-[12px] font-bold text-ink group-hover:text-lime transition-colors">Connect Stripe</p>
            ...
            <Link href="/dashboard/create" className="flex items-start gap-3 w-full group relative z-10">
               ...
               <div className="flex-1">
                  <p className="text-[12px] font-bold text-ink group-hover:text-lime transition-colors">First Product</p>
