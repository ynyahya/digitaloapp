...
export function MonoMockup({
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-2xl border border-white/[0.08] bg-gradient-to-br from-night via-night-raised to-[#1f1f1f] text-chalk shadow-card",
        ratio,
        className,
      )}
    >
      ...
      <div className="absolute inset-x-6 bottom-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-[13px] font-medium text-chalk">{label}</p>
        </div>
        <div className="flex items-end gap-1.5">
          {[22, 34, 18, 40, 28, 46, 30, 44].map((h, i) => (
            <span
              key={i}
              className="w-1.5 rounded-full bg-lime/50"
              style={{ height: `${h}px` }}
            />
          ))}
        </div>
