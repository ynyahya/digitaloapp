  ...
  return (
    <section className="py-16 md:py-24">
      <div className="mx-auto w-full max-w-[1200px] px-5 md:px-8">
        <div className="rounded-3xl border border-white/[0.08] bg-white/[0.06] p-8 md:p-12">
          <div className="flex flex-col items-start justify-between gap-6 md:flex-row md:items-end">
            <div className="max-w-xl">
              ...
              <p className="mt-3 text-[14.5px] leading-relaxed text-chalk-muted">
          ...
          <div className="mt-10 grid gap-3 md:grid-cols-3">
            {TIERS.map((t) => (
              <div
                key={t.name}
                className={
                  "relative flex flex-col rounded-2xl border bg-night p-5 " +
                  (t.popular ? "border-lime/30 shadow-card" : "border-white/[0.08]")
                }
              >
                {t.popular && (
                  <span className="absolute -top-3 right-5 inline-flex h-6 items-center rounded-full bg-lime px-3 text-[10px] font-semibold uppercase tracking-wide text-night">
                    Popular
                  </span>
                )}
                <p className="text-[14px] font-semibold">{t.name}</p>
                <p className="mt-1 text-[22px] font-semibold tracking-tight">{t.price}</p>
                <p className="mt-1 text-[12.5px] text-chalk-muted">{t.tagline}</p>
                <ul className="mt-5 flex-1 space-y-2 text-[12.5px]">
                  {t.perks.map((p) => (
                    <li key={p} className="flex items-start gap-2 text-chalk-muted">
                      <Check className="mt-0.5 h-3.5 w-3.5 text-chalk" />
                      <span>{p}</span>
                    </li>
                  ))}
                </ul>
