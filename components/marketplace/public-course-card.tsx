  ...
  return (
    <div className="group relative">
      <Link
        ...
        className="flex flex-col overflow-hidden rounded-[2rem] border border-line bg-paper transition-all duration-500 hover:border-ink/20 hover:shadow-2xl h-full"
      >
        {/* Visual Preview */}
        <div className="relative aspect-video w-full overflow-hidden bg-line">
          {course.coverImage ? (
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-paper-muted to-line/50 flex items-center justify-center">
              <Play className="h-10 w-10 text-ink/20" />
            </div>
          )}
          <div className="absolute inset-0 bg-black/10 group-hover:bg-black/0 transition-colors" />
          <div className="absolute bottom-3 left-3 flex gap-2">
            <span className="rounded-full bg-paper/90 backdrop-blur-md px-2.5 py-1 text-[11px] font-bold text-ink shadow-sm uppercase tracking-widest">
              {course.level}
            </span>
          </div>
        </div>
        ...
        <div className="flex flex-col flex-1 p-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[13px] font-bold text-ink-muted">{course.creatorName}</span>
          </div>

          <h3 className="text-[18px] font-bold tracking-tight text-ink mb-2 line-clamp-2">
            {course.title}
          </h3>

          <div className="flex items-center gap-4 text-[12px] text-ink-muted mb-6">
          ...
          <div className="mt-auto flex items-center justify-between border-t border-line/60 pt-4">
            <div className="flex items-center gap-1 text-[13px] font-bold text-amber-500">
              ...
              <span className="text-ink-muted font-normal text-[11px]">({course.ratingCount})</span>
            </div>
