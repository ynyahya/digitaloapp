  ...
  return (
    <section id="changelog" className="scroll-mt-24">
      <div className="flex items-end justify-between gap-4 border-b border-white/[0.08] pb-4">
        ...
        <div className="hidden text-right text-[11px] text-chalk-muted sm:block">
          Latest{" "}
      ...
      <ol className="mt-6 space-y-4">
        {sorted.map((entry, idx) => (
          <li
            ...
            className="relative rounded-2xl border border-white/[0.08] bg-night p-5"
          >
            <div className="flex flex-wrap items-baseline gap-3">
              <span className="inline-flex items-center rounded-full border border-white/[0.08] bg-white/[0.035] px-2.5 py-0.5 font-mono text-[11.5px] font-semibold text-chalk">
                v{entry.version}
              </span>
              <span className="text-[11.5px] text-chalk-muted">
                {formatRelativeDate(entry.date)}
              </span>
              {idx === 0 && (
                <span className="inline-flex items-center rounded-full bg-lime px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-night">
                  Latest
                </span>
              )}
            </div>
            <p className="mt-3 whitespace-pre-wrap text-[13.5px] leading-relaxed text-chalk">
              {entry.notes}
            </p>
          </li>
        ))}
